...
#pragma HLS ARRAY_PARTITION variable=ph complete dim=0
	...
	static primitive inst_prim_conv_5;
...
#pragma HLS UNROLL
	...
	if(this->pr){
		for(int i=0;i<th_ch11;i++)
			std::cout<<"th11[1][0]["<<i<<"]= "<<th11[1][0][i]<<std::endl;
	}
...
#pragma HLS UNROLL
	...
	if(this->pr){
		...
		//		std::cout<<"th[2][0]["<<i<<"]= "<<th[1][0][i]<<std::endl;
		}
	// neighbor sector single ME11 chamber
   inst_prim_conv_5.top_prim_conv(vpf[5][0],q[5][0], wg[5][0], hstr[5][0],
   					cpat[5][0], i,//station
   					j,//cscid
   					r_in, we_w[5][0], ph[5][0], th[5][0], &vl[5][0],
   					&phzvl[5][0], &me11a[2][0], cpatr[5][0], &ph_hit[5][0],
   					&th_hit[5][0], sel, addr, endcap, lat_test,print_flag, &r_out_w[i][j]);
   ...
   inst_prim_conv_neighbor[j].top_prim_conv(vpf[5][j],q[5][j], wg[5][j], hstr[5][j],
   					...
   					&th_hit[5][j], sel, addr, endcap, lat_test,print_flag, &r_out_w[i][j]);
