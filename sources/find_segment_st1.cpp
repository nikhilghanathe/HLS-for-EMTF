...
#pragma HLS ARRAY_PARTITION variable=ph_seg_p complete dim=0
   ...
   const int nodiff=31;
	...
	ap_uint<1> rk;
	 ...
	 #pragma HLS UNROLL
	 		find_segment_st1_label1:for(int j=0;j<zone_cham;j++){
	 #pragma HLS UNROLL
	 ...
	 #pragma HLS UNROLL
	 			...
	 			find_segment_st1_label3:for(int k=0;k<zone_seg;k++){
	 #pragma HLS UNROLL
	 ...
	 #pragma HLS UNROLL
	 		find_segment_st1_label6:for (j = 0; j < zone_cham; j = j+1){ // chamber loop
	 #pragma HLS UNROLL
	 ...
	 #pragma HLS UNROLL	// compare 2 values
	 			...
	 			*ph_match = ph_seg[a_hid][a_cid][a_sid]; // route best matching phi to output
	 			...
	 			find_segment_st1_label11:for(int i=0;i<zone_seg;i++)
	 			{
	 			#pragma HLS UNROLL
