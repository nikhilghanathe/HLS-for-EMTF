...
void SectorProcessor::process_single_bx(
  ...
  /****** HLS Configure********************/
	static SectorProcessor_HLS sp[2][6];
	std::cout<<"endcap= "<<endcap_<<" sector= "<<sector_<<std::endl;
   if(!sp[endcap_-1][sector_-1].lut_filled)
	   sp[endcap_-1][sector_-1].configure( lut_, endcap_, sector_, verbose_, 1/*we*/);
  ...
  /****************************HLS PrimitiveConversion*********************************/
	sp[endcap_-1][sector_-1].process(lut_, endcap_, sector_, verbose_, /*we*/0, selected_csc_map, conv_hits, best_tracks_HLS); 
  /************************************************************************************/
  ...
  out_tracks.insert(out_tracks.end(), best_tracks.begin(), best_tracks.end());
   	if(verbose_>0){
	//		std::cout<<"best_track size= "<<best_tracks.size()<<" out_tracks size= "<<out_tracks.size()<<std::endl;
